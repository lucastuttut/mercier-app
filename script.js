...
window.addEventListener('error', function(e) { 
    ...
    if(errLog) { errLog.classList.remove('hidden'); errLog.innerText += "ERRO: " + e.message + " | "; }
});
...
// --- SISTEMA DE LOGIN DISCRETO & FILTRO MINHAS TAREFAS ---
let usuarioAtual = "";
...
function calcP(d, pr){ 
    ...
    try { 
        ...
        if(spr.includes("util")){ 
            let c=0; 
            while(c < parseInt(spr)){ dF.setDate(dF.getDate()+1); if(dF.getDay()!==0 && dF.getDay()!==6) c++; } 
        } else { dF.setDate(dF.getDate() + parseInt(spr)); } 
...
// --- ABA TAREFAS/VENDA (TIRAR PEDIDO E CAIXA MÁGICA) ---
...
function verDetalhesTarefa(uid){ const t=tarefas.find(x=>x.uid==uid); if(!t) return; document.getElementById('modal-detalhes').style.display='flex'; const c=document.getElementById('detalhe-corpo'); if(!t.detalhes){ c.innerHTML=`<div class="font-black uppercase">${esc(t.descricao)}</div>`; return; } const d = t.detalhes; let enderecoCompleto = `${d.end || ''}${d.num ? ', ' + d.num : ''}${d.torre ? ' - ' + d.torre : ''}${d.bairro ? ' - ' + d.bairro : ''}${d.cidade ? ' - ' + d.cidade : ''}`; let h = `<div class="grid grid-cols-2 gap-2">${l_i("CLIENTE", d.cliente)}${l_i("CPF", d.cpf)}${l_i("CONTATO 1", d.contato)}${l_i("CONTATO 2", d.contato2 || "-")}${l_i("CEP", d.cep)}${l_i("ENDEREÇO", enderecoCompleto)}</div><div class="mt-4 font-black text-xs uppercase border-b text-blue-600">Móveis:</div>`; const prods = safeArray(d.produtos); prods.forEach(p => h += `<div class="text-xs font-bold border-b py-1 flex justify-between items-center"><span>${esc(p.n)} <span class="text-slate-400 line-through text-[10px] ml-1">${esc(p.o)}</span> <span class="text-indigo-600 ml-1">${esc(p.d)}</span></span><button onclick="copyText('${esc(p.n)} - De: ${esc(p.o)} Por: ${esc(p.d)}', this)">📋</button></div>`); h += `<div class="mt-4 font-black text-xs uppercase border-b text-emerald-600">Pagamento:</div>`; const pags = safeArray(d.pagamentos); pags.forEach(p => h += `<div class="text-xs font-bold border-b py-1 flex justify-between"><span>${esc(p.t)}: ${esc(p.v)} (${esc(p.o)})</span><button onclick="copyText('${esc(p.t)}: ${esc(p.v)}', this)">📋</button></div>`); if(d.obs && d.obs !== "") { h += `<div class="mt-4 font-black text-xs uppercase border-b text-orange-600">Observações:</div><div class="text-xs font-bold py-2 bg-slate-50 p-2 rounded mt-1">${esc(d.obs)}</div>`; } c.innerHTML = h; }
            ...
            try {
                ...
                canvas.toBlob(async function(blob) {
                    try {
                        ...
                        alert("⚠️ O navegador bloqueou a cópia da foto. O WhatsApp será aberto só com texto.");
                    }
...
function renderQuadroEquipe() {
    ...
    if(document.getElementById('col-done')) document.getElementById('col-don
