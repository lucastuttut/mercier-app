...
function renderPedidos() {
    ...
    tb.innerHTML = lista.map(x=>{
        ...
        return `<tr class="${p.classe}">
            ...
            <td onclick="editField(${x.uid},'cliente')" class="editable-cell uppercase font-tabela-fixa">${x.cliente}</td>
            <td class="font-tabela-fixa">${x.dataPedido}</td><td class="text-center font-black">${x.qtd}</td>
            <td onclick="editField(${x.uid},'produto')" class="editable-cell uppercase font-tabela-fixa">${x.produto}</td>
            <td onclick="editField(${x.uid},'medida')" class="editable-cell font-tabela-fixa">${x.medida}</td>
            <td onclick="editField(${x.uid},'cor')" class="editable-cell font-tabela-fixa uppercase">${x.cor}</td>
...
function dupPed(u){
    ...
    pedidos.unshift({...x, uid:Date.now(), idDoc}); salvarCloud();
}
...
// ... DEMAIS FUNÇÕES DE TAREFAS, CESTO E COMPLEMENTARES MANTIDAS ...
...
function mostrarCamposTarefa(t){
    ...
    if(t==='TIRAR PEDIDO'){
        c.innerHTML=`<input id="t_nome" placeholder="CLIENTE" class="border-2 p-2 rounded-lg text-xs font-bold col-span-2"><input id="t_cpf" placeholder="CPF" class="border-2 p-2 rounded-lg text-xs font-bold" oninput="maskCPF(this)"><input id="t_contato" placeholder="CONTATO" class="border-2 p-2 rounded-lg text-xs font-bold"><input id="t_cep" placeholder="CEP" class="border-2 p-2 rounded-lg text-xs font-bold" oninput="buscarCEP(this)"><input id="t_end" placeholder="RUA" class="border-2 p-2 rounded-lg text-xs font-bold col-span-2"><input id="t_bairro" placeholder="BAIRRO" class="border-2 p-2 rounded-lg text-xs font-bold"><input id="t_cidade" placeholder="CIDADE" class="border-2 p-2 rounded-lg text-xs font-bold"><input id="t_num" placeholder="NÚMERO" class="border-2 p-2 rounded-lg text-xs font-bold"><input id="t_torre" placeholder="TORRE" class="border-2 p-2 rounded-lg text-xs font-bold"><div class="col-span-4 border-t mt-2 pt-2"><div id="lista-produtos-tarefa"></div><button onclick="addProdutoLinha()" class="text-[10px] font-black text-blue-600 mt-2 uppercase">+ MÓVEL</button></div><div class="col-span-4 border-t mt-2 pt-2"><div id="lista-pagamentos-tarefa"></div><button onclick="addPagamentoLinha()" class="text-[10px] font-black text-blue-600 mt-2 uppercase">+ PAGAMENTO</button></div><textarea id="t_obs" placeholder="OBS" class="col-span-4 border-2 p-2 rounded-lg text-xs font-bold h-16 uppercase"></textarea>`;
        addProdutoLinha(); addPagamentoLinha();
    } else { c.innerHTML = `<input id="t_raw" placeholder="DESCRIÇÃO..." class="border-2 p-2 rounded-lg text-xs font-bold col-span-4">`; }
}
function addProdutoLinha(){ const d = document.getElementById('lista-produtos-tarefa'); const r = document.createElement('div'); r.className = "flex gap-2 mb-1 items-center row-prod"; r.innerHTML = `<input class="t-p-nome border-2 p-2 rounded text-xs font-bold flex-1" placeholder="MÓVEL"><input class="t-v-orig border-2 p-2 rounded text-xs font-bold w-24" placeholder="ORIGINAL" oninput="maskMoney(this)"><input class="t-v-desc border-2 p-2 rounded text-xs font-bold w-24" placeholder="DESC." oninput="maskMoney(this)"><button onclick="this.parentElement.remove(); calcTotalTirarPedido();" class="text-red-500 font-black px-2">✕</button>`; d.appendChild(r); }
...
function calcTotalTirarPedido(){ /* calculo opcional visual */ }
...
function verDetalhesTarefa(uid){
    ...
    if(!t.detalhes){ c.innerHTML=`<div class="font-black">${t.descricao}</div>`; return; }
    const d = t.detalhes;
    let h = `<div class="grid grid-cols-2 gap-2">${l_i("CLIENTE", d.cliente)}${l_i("CPF", d.cpf)}${l_i("CONTATO", d.contato)}${l_i("ENDEREÇO", d.end)}</div>`;
    h += `<div class="mt-4 font-black">MÓVEIS:</div>`;
    d.produtos.forEach(p => h += `<div class="text-xs">${p.nome} - ${p.desc} <button onclick="copyText('${p.nome} - ${p.desc}')">📋</button></div>`);
    c.innerHTML = h;
}
function l_i(l, v){ return `<div class="border p-2 rounded text-xs font-bold uppercase"><div>${l}:</div>${v} <button onclick="copyText('${v}')">📋</button></div>`; }
