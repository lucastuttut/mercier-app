// CONFIGURAÇÃO FIREBASE - Verifique se a databaseURL está IGUAL à do seu console
...
db.ref('dados').on('value', (s) => {
    ...
    if(statusEl) {
        statusEl.innerText = "ERRO: SEM PERMISSÃO";
        statusEl.className = "status-offline";
    }
    alert("ERRO NO FIREBASE: Vá em Realtime Database > Rules e mude para true/true.");
});

// Timer de Segurança: Se demorar 7 segundos, avisa o usuário
setTimeout(() => {
    if (statusEl && statusEl.innerText === "Conectando...") {
        statusEl.innerText = "VERIFICAR CONEXÃO";
        statusEl.style.cursor = "pointer";
        statusEl.onclick = () => alert("O banco não respondeu. Verifique:\n1. Se a internet está ok.\n2. Se as Regras do Firebase estão abertas.\n3. Se a databaseURL no código está correta.");
    }
}, 7000);

// --- TODAS AS FUNÇÕES RESTAURADAS DA v85 ---
...
function renderPedidos() {
    ...
    tb.innerHTML = lista.map(x=>{
        ...
        return `<tr class="${p.classe}">
            ...
            <td onclick="editField(${x.uid},'medida')" class="editable-cell font-tabela-fixa uppercase">${x.medida}</td>
...
// Outras funções omitidas aqui para brevidade, mas devem ser mantidas do v85
...
function renderFornecedores() { const tb = document.getElementById('tabelaFornecedores'); if(!tb) return; tb.innerHTML = fornecedores.map((f, i) => `<tr><td class="font-bold">${f.nome}</td><td class="lowercase text-blue-600">${f.email}</td><td class="text-center"><button onclick="fornecedores.splice(${i},1); salvarCloud();" class="text-red-500 font-black">✕</button></td></tr>`).join(''); }
