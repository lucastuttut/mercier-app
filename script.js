...
document.onmousemove = (e) => {
    if (isDragging) {
        ...
        dragPanel.style.bottom = 'auto'; // Remove trava do bottom original
        dragPanel.style.right = 'auto'; // Remove trava do right original
    }
};

// --- EDICÃO CLICÁVEL (PEDIDOS E ESTOQUE) ---
...
function renderEstoque() {
    ...
    tb.innerHTML = lista.map(x => `
        <tr>
            ...
            <td onclick="editFieldEstoque(${x.uid},'qtd')" class="editable-cell">${x.qtd}</td>
            ...
            <td class="text-center"><button onclick="if(confirm('EXCLUIR?')){estoque=estoque.filter(y=>y.uid!=${x.uid}); salvarCloud();}" class="text-red-500 font-black">✕</button></td>
        </tr>`).join('');
}
...
function cadastrarEstoque() {
    ...
    if (p) { estoque.push({ uid: Date.now(), produto: p, fabrica: f, qtd: q, situacao: s }); salvarCloud(); document.getElementById('e_produto').value = ""; }
}
...
// --- MANTENDO O RESTANTE DO SCRIPT COMPLETO ---
