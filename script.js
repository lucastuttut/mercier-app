...
function copyText(v){ navigator.clipboard.writeText(v.toLowerCase().replace(/(^\w{1})|(\s+\w{1})/g, l => l.toUpperCase())); alert("Copiado!"); }
function maskCPF(i){ let v=i.value.replace(/\D/g,""); if(v.length>11)v=v.slice(0,11); v=v.replace(/(\d{3})(\d)/,"$1.$2"); v=v.replace(/(\d{3})(\d)/,"$1.$2"); v=v.replace(/(\d{3})(\d{1,2})$/,"$1-$2"); i.value=v; verifCPF(i); }
...
function adicionarItemAoCesto() {
    ...
    if(!prod) return alert("Preecha o produto!");
...
function renderCesto() {
    document.getElementById('cesto-itens').innerHTML = cestoItensTemporario.map((item, idx) => `
        <div class="item-cesto">
            <span class="font-black">${item.q}x</span>
            <span>${item.p}</span>
            <button onclick="cestoItensTemporario.splice(${idx},1); renderCesto();" class="text-red-500">✕</button>
        </div>
    `).join('');
}
...
function cadastrarManual() {
    ...
    if(!cli || cestoItensTemporario.length === 0) return alert("Preencha cliente e itens!");
...
function renderPedidos() {
    ...
    tb.innerHTML = l.map(x=>{
        ...
        <tr class="${p.classe}">
            ...
            <td><div class="flex flex-col items-center"><span class="text-[9px] font-black">${p.dias}D</span>
            ...
            <td onclick="editField(${x.uid},'cliente')" class="editable-cell col-cliente font-tabela-fixa uppercase">${x.cliente}</td>
            <td class="font-tabela-fixa">${x.dataPedido}</td>
            <td class="text-center font-black">${x.qtd}</td>
            <td class="col-movel font-tabela-fixa uppercase">${x.produto}</td>
            ...
            <td class="font-black text-blue-800 text-[10px] uppercase">${x.fornecedor}</td>
            ...
            <td class="text-center flex gap-2 justify-center">
                ...
                <button onclick="excluirPedido(${x.uid})" class="text-red-500">✖</button>
            </td>
        </tr>`;
    }).join('');
}
...
function mostrarCamposTarefa(t){
    ...
    if(t==='TIRAR PEDIDO'){
        c.innerHTML=`<input id="t_nome" placeholder="NOME" class="border-2 p-2 rounded-lg text-xs font-bold"><input id="t_cpf" placeholder="CPF" class="border-2 p-2 rounded-lg text-xs font-bold" oninput="maskCPF(this)"><input id="t_contato" placeholder="CONTATO" class="border-2 p-2 rounded-lg text-xs font-bold"><input id="t_end" placeholder="ENDEREÇO" class="border-2 p-2 rounded-lg text-xs font-bold col-span-2"><textarea id="t_obs" placeholder="MÓVEIS E VALORES" class="col-span-3 border-2 p-2 rounded text-xs font-bold h-20 uppercase"></textarea>`;
    } else c.innerHTML = `<input id="t_raw" placeholder="DESCRIÇÃO..." class="border-2 p-3 rounded-xl text-xs font-bold col-span-3">`;
}
...
function renderTarefas() {
    ...
    document.getElementById('tabelaTarefas').innerHTML=l.map(x=>`
        <tr onclick="verDetalhesTarefa(${x.uid})" class="hover:bg-slate-50 cursor-pointer">
            ...
            <td class="text-center"><button onclick="event.stopPropagation(); if(confirm('Excluir?')){tarefas=tarefas.filter(y=>y.uid!=${x.uid});salvarCloud();}" class="text-red-500">✖</button></td>
        </tr>`).join('');
}
function verDetalhesTarefa(uid){
    ...
    c.innerHTML = `<div class="grid grid-cols-1 gap-2">
        <div class="bg-slate-50 p-3 rounded border text-xs font-black uppercase">CLIENTE: ${t.detalhes.nome} <br> CONTATO: ${t.detalhes.contato}</div>
...
function editField(u,f){ const x=pedidos.find(y=>y.uid==u); let n=prompt(`Editar:`,x[f]||""); if(n!==null){ x[f]=n.toUpperCase(); salvarCloud(); } }
...
function renderCatalogo() { document.getElementById('tabelaCatalogo').innerHTML=catalogo.map((c,i)=>`<tr><td>${c.nome}</td><td><button onclick="catalogo.splice(${i},1);salvarCloud();" class="text-red-500">✖</button></td></tr>`).join(''); }
function renderFornecedores() { document.getElementById('tabelaFornecedores').innerHTML=fornecedores.map((f,i)=>`<tr><td>${f.nome}</td><td class="lowercase">${f.email}</td><td><button onclick="fornecedores.splice(${i},1);salvarCloud();" class="text-red-500">✖</button></td></tr>`).join(''); }
