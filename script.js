...
// --- LÓGICA DE ARRASTAR BLOCO DE NOTAS ---
const dragPanel = document.getElementById('painel-sugestoes');
const dragHandle = document.getElementById('drag-handle');
let isDragging = false, offset = [0,0];

dragHandle.onmousedown = (e) => {
    isDragging = true;
    offset = [dragPanel.offsetLeft - e.clientX, dragPanel.offsetTop - e.clientY];
};
document.onmouseup = () => isDragging = false;
document.onmousemove = (e) => {
    if (isDragging) {
        dragPanel.style.left = (e.clientX + offset[0]) + 'px';
        dragPanel.style.top  = (e.clientY + offset[1]) + 'px';
        dragPanel.style.bottom = 'auto'; dragPanel.style.right = 'auto';
    }
};
...
function renderEstoque() {
    ...
    let totalEstoque = 0, totalVendidos = 0;

    let lista = estoque.filter(x => {
        if(x.situacao === 'ESTOQUE') totalEstoque += parseInt(x.qtd || 0);
        if(x.situacao === 'VENDIDO') totalVendidos += parseInt(x.qtd || 0);

        const matchesBusca = (x.produto || "").toLowerCase().includes(busca) || (x.fabrica || "").toLowerCase().includes(busca);
    ...
    document.getElementById('resumo-estoque-total').innerText = totalEstoque;
    document.getElementById('resumo-estoque-vendidos').innerText = totalVendidos;
    ...
    tb.innerHTML = lista.map(x => `
        <tr>
            ...
            <td class="text-center flex gap-1 justify-center">
                ${x.situacao === 'ESTOQUE' ? `<button onclick="darBaixaEstoque(${x.uid})" class="text-emerald-600 font-black px-2" title="Dar Saída">📉</button>` : ''}
...
function darBaixaEstoque(uid) {
    const item = estoque.find(x => x.uid == uid);
    if (!item) return;
    
    let qtdSaida = prompt(`Quantas unidades de "${item.produto}" saíram? (Em estoque: ${item.qtd})`, "1");
    if (qtdSaida === null) return;
    qtdSaida = parseInt(qtdSaida);
    
    if (isNaN(qtdSaida) || qtdSaida <= 0 || qtdSaida > item.qtd) {
        return alert("Quantidade de saída inválida ou superior ao estoque!");
    }

    if (qtdSaida == item.qtd) {
        // Se saiu tudo, apenas muda o status
        item.situacao = "VENDIDO";
        item.data = new Date().toLocaleDateString('pt-BR');
    } else {
        // Se saiu parte, subtrai do estoque e cria nova linha de vendido
        item.qtd -= qtdSaida;
        estoque.unshift({
            uid: Date.now(),
