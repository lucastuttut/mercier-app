...
function mostrarCamposTarefa(t){
    ...
    if(t==='TIRAR PEDIDO'){
        c.innerHTML=`
            ...
            <input id="t_torre" placeholder="TORRE / APTO" class="border-2 p-2 rounded-lg text-xs font-bold uppercase">
            
            <div class="col-span-4 border-t mt-4 pt-4">
                <div id="lista-produtos-tarefa"></div>
                <button onclick="addProdutoLinha()" class="text-[10px] font-black text-blue-600 mt-2 uppercase">+ ADICIONAR MÓVEL</button>
                <div id="total-pedido-tarefa" class="text-right text-indigo-600 font-black text-sm mt-1 uppercase italic">Total c/ Desconto: R$ 0,00</div>
            </div>

            <div class="col-span-4 border-t mt-4 pt-4">
                <div id="lista-pagamentos-tarefa"></div>
                <button onclick="addPagamentoLinha()" class="text-[10px] font-black text-emerald-600 mt-2 uppercase">+ ADICIONAR PAGAMENTO</button>
            </div>

            <textarea id="t_obs" placeholder="OBSERVAÇÕES DO CONTRATO" class="col-span-4 border-2 p-2 rounded-lg text-xs font-bold h-20 uppercase outline-indigo-500"></textarea>
        `;
        addProdutoLinha(); addPagamentoLinha();
    } else {
        c.innerHTML = `<input id="t_raw" placeholder="DESCREVA A TAREFA..." class="border-2 p-3 rounded-xl text-xs font-bold col-span-4 uppercase outline-indigo-500">`;
    }
}
...
function addProdutoLinha(){
    ...
    r.innerHTML=`<input class="t-p-nome border-2 p-2 rounded text-xs font-bold flex-1 uppercase" placeholder="NOME DO MÓVEL"><input class="t-v-orig border-2 p-2 rounded text-xs font-bold w-32" placeholder="VALOR ORIGINAL" oninput="maskMoney(this)"><input class="t-v-desc border-2 p-2 rounded text-xs font-bold w-32 text-indigo-600" placeholder="VALOR FINAL" oninput="maskMoney(this)"><button onclick="this.parentElement.remove(); calcTotalTirarPedido();" class="text-red-500 font-black px-2">✕</button>`;
    d.appendChild(r);
}
...
function addPagamentoLinha(){
    ...
    r.innerHTML=`
        ...
        <div class="flex gap-2">
            <input class="t-p-val border-2 p-2 rounded-lg text-xs font-bold w-48 text-emerald-600" placeholder="VALOR" oninput="maskMoney(this)" value="R$ ${saldo.toLocaleString('pt-BR',{minimumFractionDigits:2})}">
            <input class="t-p-obs border-2 p-2 rounded-lg text-xs font-bold flex-1 uppercase" placeholder="DATA / PARCELAS / OBS">
...
function calcTotalTirarPedido(){
    ...
    document.getElementById('total-pedido-tarefa').innerText="Total c/ Desconto: R$ "+t.toLocaleString('pt-BR',{minimumFractionDigits:2});
}
...
function cadastrarTarefa(){
    ...
    if(t==='TIRAR PEDIDO'){
        const cli=document.getElementById('t_nome').value; if(!cli) return alert("FALTA O NOME DO CLIENTE!");
        let total=0; document.querySelectorAll('.t-v-desc').forEach(i=>total+=parseMoney(i.value));
        ...
        document.querySelectorAll('.row-pag').forEach(r=>{
            obj.detalhes.pagamentos.push({ t:r.querySelector('.t-p-tipo').value, v:r.querySelector('.t-p-val').value, o:r.querySelector('.t-p-obs').value.toUpperCase() });
        });
    } else { obj.descricao=(document.getElementById('t_raw')?.value||"").toUpperCase(); }
    
    if(!obj.descricao) return alert("PREENCHA A TAREFA!");
...
function verDetalhesTarefa(uid){
    ...
    let h=`<div class="grid grid-cols-2 gap-3">
        ${l_i("CLIENTE", d.cliente)} ${l_i("CPF", d.cpf)} ${l_i("CELULAR", d.contato)} ${l_i("CEP", d.cep)}
        ${l_i("ENDEREÇO", d.end)} ${l_i("Nº/TORRE", (d.num+' '+(d.torre||'')))} ${l_i("BAIRRO", d.bairro)} ${l_i("CIDADE", d.cidade)}
    </div>
    <div class="mt-6 font-black text-[11px] border-b-2 pb-1 uppercase tracking-tighter text-blue-600">Móveis e Valores</div>`;
    d.produtos.forEach(p=> h+=`<div class="flex justify-between items-center bg-slate-50 p-2 rounded-lg border mb-1 text-xs font-bold">
        <span>${p.n}</span> <span class="text-slate-400 font-normal">De: ${p.o} / <b class="text-indigo-600">Por: ${p.d}</b></span> <button class="copy-btn-small" onclick="copyText('${p.n} - ${p.d}', this)">📋</button>
    </div>`);
    h+=`<div class="text-right font-black text-indigo-700 uppercase italic">Total Venda: ${d.totalDesc}</div>
    <div class="mt-6 font-black text-[11px] border-b-2 pb-1 uppercase tracking-tighter text-emerald-600">Pagamento</div>`;
    d.pagamentos.forEach(p=> h+=`<div class="flex justify-between items-center bg-slate-50 p-2 rounded-lg border mb-1 text-xs font-bold">
        <span>${p.t}: <b class="text-emerald-600">${p.v}</b></span> <span class="text-[10px] text-slate-400 uppercase">${p.o}</span> <button class="copy-btn-small" onclick="copyText('${p.t} - ${p.v}', this)">📋</button>
    </div>`);
    h+=`<div class="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-xl text-xs font-bold relative uppercase">
        <div class="text-[9px] text-amber-500 mb-1">Observações do Contrato</div> ${d.obs || "Nenhuma"} <button class="absolute top-2 right-2 copy-btn-small" onclick="copyText('${d.obs}', this)">📋</button>
    </div>`;
    c.innerHTML=h;
}

function l_i(l, v){ return `<div class="bg-white p-2 rounded border flex justify-between items-center group transition hover:border-blue-400 shadow-sm"><div><div class="text-[8px] text-slate-400 font-black mb-0.5">${l}</div><div class="text-[11px] font-black uppercase text-slate-800">${v || '-'}</div></div><button class="copy-btn-small opacity-0 group-hover:opacity-100" onclick="copyText('${v}', this)">📋</button></div>`; }

// --- OUTRAS FUNÇÕES MANTIDAS ---
function renderPedidos() { const tb=document.getElementById('tabelaPedidos'); if(!tb) return; const b=document.getElementById('busca').value.toLowerCase(); let lista=pedidos.filter(x=>(x.cliente||"").toLowerCase().includes(b)||(x.produto||"").toLowerCase().includes(b)||(x.idDoc||"").toLowerCase().includes(b)||(x.fornecedor||"").toLowerCase().includes(b)); if(filtrandoNaoEnviados) lista=lista.filter(x=>x.status==="Não enviado"); document.getElementById('contador').innerText=lista.length+" PEDIDOS"; tb.innerHTML = lista.map(x=>{ const p=calcP(x.dataPedido, x.prazo); let sCls = x.status==="Não enviado" ? "bg-red-600" : (x.status.includes("loja") ? "bg-green-700" : "bg-blue-600"); return `<tr class="${p.classe}"> <td><input type="checkbox" class="ped-check" value="${x.uid}"></td> <td><div class="flex flex-col gap-1 items-center"><span class="font-black text-[9px]">${p.dias}D</span><select onchange="updPed(${x.uid},'prazo',this.value)" class="select-prazo-tabela"><option value="15" ${x.prazo=='15'?'selected':''}>15C</option><option value="20" ${x.prazo=='20'?'selected':''}>20C</option><option value="30" ${x.prazo=='30'?'selected':''}>30C</option><option value="30-util" ${x.prazo=='30-util'?'selected':''}>30U</option><option value="40-util" ${x.prazo=='40-util'?'selected':''}>40U</option></select></div></td> <td class="text-[10px] text-slate-400 font-black">${x.idDoc}</td> <td onclick="editField(${x.uid},'cliente')" class="editable-cell uppercase font-tabela-fixa">${x.cliente}</td> <td onclick="editField(${x.uid},'dataPedido')" class="editable-cell font-tabela-fixa">${x.dataPedido}</td> <td onclick="editField(${x.uid},'qtd')" class="editable-cell text-center font-black">${x.qtd}</td> <td onclick="editField(${x.uid},'produto')" class="editable-cell uppercase font-tabela-fixa">${x.produto}</td> <td onclick="editField(${x.uid},'medida')" class="editable-cell font-tabela-fixa uppercase">${x.medida}</td> <td onclick="editField(${x.uid},'cor')" class="editable-cell font-tabela-fixa uppercase">${x.cor}</td> <td onclick="editField(${x.uid},'custo')" class="editable-cell font-tabela-fixa">${x.custo}</td> <td onclick="editField(${x.uid},'fornecedor')" class="editable-cell font-black text-blue-800 uppercase text-[10px]">${x.fornecedor}</td> <td><button onclick="cycleStatus(${x.uid})" class="status-badge ${sCls} text-white">${x.status}</button></td> <td><button onclick="togPed(${x.uid},'whatsEnviado')" class="status-badge ${x.whatsEnviado?'btn-sim':'btn-nao'}">${x.whatsEnviado?'SIM':'NÃO'}</button></td> <td><button onclick="togPed(${x.uid},'confirmado')" class="status-badge ${x.confirmado?'btn-sim':'btn-nao'}">${x.confirmado?'SIM':'NÃO'}</button></td> <td class="text-center flex gap-1 justify-center"> <button onclick="copyText('${x.qtd}x ${x.produto} ${x.cor} (${x.idDoc})')">📋</button> <button onclick="dupPed(${x.uid})">➕</button> <button onclick="gerarAssistenciaRapida(${x.uid})">🛠️</button> <button onclick="excluirPedido(${x.uid})" class="text-red-500 font-black">✕</button> </td></tr>`; }).join(''); }
