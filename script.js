...
function renderEstoque() {
    ...
    tb.innerHTML = lista.map(x => `
        <tr>
            ...
            <td class="text-center"><button onclick="if(confirm('EXCLUIR?')){estoque=estoque.filter(y=>y.uid!=${x.uid}); salvarCloud();}" class="text-red-500">✕</button></td>
        </tr>`).join('');
}
