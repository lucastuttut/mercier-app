...
db.ref('dados').on('value', (s) => {
    ...
    atualizarSugestoes(); atualizarSelectsFornecedores(); renderAll(); calcularDashboard();
});
...
function calcularDashboard() {
    let custoPendente = 0; let custoProducao = 0; let qtdLoja = 0;
    pedidos.forEach(p => {
        let v = parseMoney(p.custo);
        if(p.status === "Não enviado") custoPendente += v;
        else if(p.status.includes("enviado") || p.status.includes("Aguardando")) custoProducao += v;
        if(p.status.includes("loja")) qtdLoja++;
    });
    document.getElementById('dash-custo-pendente').innerText = "R$ " + custoPendente.toLocaleString('pt-BR', {minimumFractionDigits: 2});
    document.getElementById('dash-custo-producao').innerText = "R$ " + custoProducao.toLocaleString('pt-BR', {minimumFractionDigits: 2});
    document.getElementById('dash-qtd-loja').innerText = qtdLoja + " Itens";
}

// --- PDF INTELLIGENCE (Beta) ---
async function lerPDF(input) {
    ...
    reader.onload = async function() {
        ...
        status.innerText = "PDF Lido! Tentando identificar dados...";
        // Lógica simples de busca no texto
        if(fullText.includes("Cliente:")) {
             let partes = fullText.split("Cliente:");
             let nome = partes[1].split("   ")[0].trim();
             document.getElementById('m_cliente').value = nome.toUpperCase();
        }
        alert("Texto extraído com sucesso! Verifique se os campos foram preenchidos.");
    };
...
function renderCesto() { document.getElementById('cesto-itens').innerHTML = cestoItensTemporario.map((item, idx) => `<div class="item-cesto"><span class="font-black text-blue-600">${item.q}x</span><span>${item.p}</span><button onclick="cestoItensTemporario.splice(${idx},1); renderCesto();" class="text-red-500 font-black">✕</button></div>`).join(''); }
...
function renderPedidos() {
    const tb=document.getElementById('tabelaPedidos'); const busca=document.getElementById('busca').value.toLowerCase();
...
function renderTarefas() {
    ...
    document.getElementById('tabelaTarefas').innerHTML=l.map(x=>`<tr onclick="verDetalhesTarefa(${x.uid})" class="hover:bg-slate-50 cursor-pointer"><td>${x.data}</td><td class="font-black text-xs uppercase">${x.descricao}</td><td class="text-[10px] font-bold">${x.tipo}</td><td><button onclick="event.stopPropagation(); cycleTarefaStatus(${x.uid})" class="status-badge bg-slate-200">${x.status}</button></td><td class="text-center"><button onclick="event.stopPropagation(); if(confirm('Excluir?')){tarefas=tarefas.filter(y=>y.uid!=${x.uid});salvarCloud();}" class="text-red-500 font-black">✖</button></td></tr>`).join('');
}
function verDetalhesTarefa(uid){
    ...
    c.innerHTML = `<div class="grid grid-cols-1 gap-2"><div class="bg-slate-50 p-3 rounded border text-xs font-black uppercase">CLIENTE: ${t.detalhes.nome} - ${t.detalhes.contato}</div><div class="bg-slate-50 p-3 rounded border text-xs font-black uppercase">CPF: ${t.detalhes.cpf}</div><div class="bg-slate-50 p-3 rounded border text-xs font-black uppercase">ENDEREÇO: ${t.detalhes.end}</div><div class="bg-slate-100 p-4 rounded border text-xs font-bold uppercase whitespace-pre-wrap">${t.detalhes.obs}</div></div>`;
}
...
function gerarAssistenciaRapida(u){ const p=pedidos.find(x=>x.uid==u); if(p){ document.getElementById('as_cliente').value=p.cliente; document.getElementById('as_produto').value=p.produto+" (DEFEITO)"; document.getElementById('as_fabrica').value=p.fornecedor; switchTab('assistencia'); alert("Dados preenchidos!"); } }
