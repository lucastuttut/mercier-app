...
function adicionarItemAoCesto() { 
    ...
    cestoItensTemporario.push({ 
        ...
        p: p, 
        m: val('m_medida') || "-", 
        c: val('m_cor').toUpperCase() || "-", 
        v: val('m_custo') || "R$ 0,00" 
    }); 
    renderCesto(); 
    getEl('m_produto').value = ""; 
}
...
async function cadastrarManual() { 
    const cli = val('m_cliente').trim().toUpperCase(); 
    const forn = val('m_fornecedor_select'); 
...
// =========================================================
// 7. ABA: LOGÍSTICA E AGENDA UNIFICADA
// =========================================================
    ...
    if(mostraEnt) {
        entregas.forEach(function(e) {
            ...
            eventosDoMes[e.dataOrdem].push({ 
                tipo: 'ENTREGA', obj: e, 
                cor: e.status === 'Entregue' ? 'bg-emerald-200 text-emerald-800 opacity-60 line-through border-emerald-300' : 'bg-emerald-500 text-white border-emerald-600', 
    ...
    if(mostraLem) {
        lembretes.forEach(function(l) {
            ...
            eventosDoMes[l.dataOrdem].push({ 
                tipo: 'LEMBRETE', obj: l, 
                cor: l.concluido ? 'bg-slate-200 text-slate-500 opacity-60 line-through' : cM[l.categoria], 
...
function renderCalendario() {
    ...
    for (let d=1; d<=dM; d++) {
        ...
        if(eventosDoMes[dK]) { 
            ...
            if(eventosDoMes[dK].length > 3) {
                eH += `<div class="text-[8px] font-black text-slate-400 text-center">+${eventosDoMes[dK].length-3} itens</div>`; 
            }
        }
        ...
        <div onclick="selecionarDiaLogistica('${dK}', '${d} de ${nm[m]}')" class="border-b border-r border-slate-200 p-1.5 min-h-[80px] transition cursor-pointer flex flex-col gap-1 ${bClass}">
            <div class="flex justify-end">
                <span class="w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-black ${cClass}">${d}</span>
            </div>
            <div class="flex-1 overflow-hidden flex flex-col gap-0.5">${eH}</div>
        </div>`;
    }
    const grid = getEl('grid-dias'); 
    if(grid) grid.innerHTML = html;
}

function mudarMes(dr) { 
    dataAtualCalendario.setMonth(dataAtualCalendario.getMonth() + dr); 
    processarEventosCalendario(); 
}

function selecionarDiaLogistica(dK, dtF) { 
    dataSelecionadaLogistica = dK; 
    renderCalendario(); 
    renderPainelDia(); 
}
...
function renderPainelDia() {
    ...
    if(evs.length===0) { 
        pn.innerHTML = '<div class="text-center p-8 border-2 border-dashed border-slate-200 rounded-2xl"><span class="text-3xl block mb-2">🎈</span><span class="text-xs font-black text-slate-400 uppercase">Dia Livre!</span></div>'; 
        return; 
    }
...
function abrirModalNovaEntrega() { 
    ...
    getEl('ent_hora').value=""; 
    getEl('ent_contato').value=""; 
    getEl('ent_cliente').value=""; 
    getEl('ent_endereco').value=""; 
    getEl('ent_produtos').value=""; 
    getEl('modal-nova-entrega').style.display='flex'; 
}

function editarEntrega(uid) {
    const e = entregas.find(function(x){ return x.uid == uid; }); 
    if(!e) return;
    getEl('titulo-modal-entrega').innerText = "✏️ Editar Entrega"; 
    getEl('ent_uid').value = e.uid; 
    getEl('ent_data').value = e.dataOrdem; 
    getEl('ent_hora').value = e.hora || ""; 
    getEl('ent_cliente').value = e.cliente; 
    getEl('ent_contato').value = e.contato || ""; 
    getEl('ent_endereco').value = e.endereco; 
    getEl('ent_produtos').value = e.produtos;
    getEl('modal-nova-entrega').style.display='flex';
}

function buscarDadosInteligentes() { 
    const n = val('ent_cliente').toUpperCase().trim(); 
    if(!n) return; 
    ...
    if(t) {
        if(!eE.value) { 
            const num = t.detalhes.num?`, ${t.detalhes.num}`:''; 
            const br = t.detalhes.bairro?` - ${t.detalhes.bairro}`:''; 
            const cd = t.detalhes.cidade?` - ${t.detalhes.cidade}`:''; 
            eE.value = `${t.detalhes.end}${num}${br}${cd}`.toUpperCase(); 
            f = true; 
        } 
        if(!eC.value && t.detalhes.contato) { 
            eC.value = t.detalhes.contato; f = true; 
        }
    }
    
    const ps = pedidos.filter(function(p) { return p.cliente && p.cliente.includes(n) && !p.finalizado; }); 
    const eP = getEl('ent_produtos'); 
    if(ps.length>0 && !eP.value) { 
        eP.value = ps.map(function(p) { return `${p.qtd}x ${p.produto} ${p.cor}`; }).join('\n').toUpperCase(); 
        f = true; 
    } 
...
function salvarEntrega() { 
    ...
    if(uid) {
        ...
        if(it) { 
            it.dataOrdem = d; it.hora = h; it.cliente = c; it.endereco = e; it.produtos = p; it.contato = cont; 
        }
    } else {
...
function excluirEntrega(uid) { 
    if(confirm("Deseja apagar esta entrega?")) { 
        entregas = entregas.filter(function(x){ return x.uid != uid; }); 
        salvarColecao('entregas', entregas); 
        processarEventosCalendario(); 
    } 
}
...
function abrirModalNovoLembrete() { 
    getEl('lem_data').value=dataSelecionadaLogistica; 
    getEl('modal-novo-lembrete').style.display='flex'; 
}
...
function excluirLembrete(uid) { 
    if(confirm("Apagar lembrete?")) { 
        lembretes = lembretes.filter(function(x){ return x.uid != uid; }); 
        salvarColecao('lembretes', lembretes); 
        processarEventosCalendario(); 
    } 
}
...
function fecharModaisLogistica() { 
    getEl('modal-nova-entrega').style.display='none'; 
    getEl('modal-novo-lembrete').style.display='none'; 
    getEl('ent_uid').value=""; 
    getEl('ent_cliente').value=""; 
    getEl('ent_endereco').value=""; 
    getEl('ent_produtos').value=""; 
...
function autoSalvarNotasEstoque() { 
    notasEstoque = val('estoque-notas-gerais'); 
...
function renderEstoque() {
    ...
    const b = noAcc(val('estoque-busca').toLowerCase()); 
    const fF = val('estoque-filtro-fabrica')||"TODAS"; 
    let tE=0, tV=0;
...
function abrirModalSaidaEstoque(uid) { 
    ...
    getEl('saida-uid').value=uid; 
    getEl('saida-produto-nome').innerText=i.produto; 
    getEl('saida-qtd').value=i.qtd; 
    getEl('saida-qtd').max=i.qtd; 
    getEl('saida-obs').value=""; 
    getEl('saida-motivo').value="VENDIDO"; 
    getEl('modal-saida-estoque').style.display='flex'; 
}
...
function confirmarSaidaEstoque() { 
    ...
    if(qS==it.qtd){ 
        it.situacao=m; it.produto=nF; it.data=new Date().toLocaleDateString('pt-BR'); 
    } else { 
        it.qtd-=qS; 
        estoque.unshift({ uid:Date.now(), data:new Date().toLocaleDateString('pt-BR'), produto:nF, fabrica:it.fabrica, qtd:qS, situacao:m }); 
    } 
    
    salvarColecao('estoque', estoque); 
    registrarAcao('📉', `SAÍDA: ${m}`, `${qS}x ${it.produto}`); 
    getEl('modal-saida-estoque').style.display='none'; 
}

function processarEstoqueMassa() { 
    const txt = val('e_massa'); 
    if(!txt||txt.trim()==="") return alert("Cole a lista na caixa mágica!"); 
    ...
    lins.forEach(function(l) { 
        ...
        let pts=l.split(/;| , /); 
        let p1=pts[0]?pts[0].trim().toUpperCase():"", p2=pts[1]?pts[1].trim().toUpperCase():"", p3=pts[2]?pts[2].trim().toUpperCase():""; 
        
        let q=1, pT=p1; 
        let mQ=p1.match(/^(\d+)\s*(?:X|\-|UN|PÇ|MÓD)?\s+(.+)/i); 
        if(mQ && parseInt(mQ[1])>0){ q=parseInt(mQ[1]); pT=mQ[2].trim(); } 
        
        let f=p2, oS=p3, st="ESTOQUE"; 
        if(oS) { 
            if(oS.includes("VENDID")){ 
                st="VENDIDO"; let qm=oS.replace(/VENDID[AOOS]*\s*(PARA|P\/)?\s*/i, '').trim(); 
                pT+=qm?" (VENDIDO: "+qm+")":" (VENDIDO)"; 
            } 
        ...
        if(pT) { 
            estoque.unshift({uid:Date.now()+Math.random(), data:new Date().toLocaleDateString('pt-BR'), produto:pT, fabrica:f, qtd:q, situacao:st}); 
            ad++; 
        } 
    }); 
    ...
    if(ad>0) { 
        salvarColecao('estoque', estoque); 
        registrarAcao('📦', 'CADASTRO EM MASSA', `${ad} itens.`); 
        getEl('e_massa').value = ""; 
        alert(`✅ ${ad} itens cadastrados!`); 
    } 
}
...
function toggleFiltroEquipe(n) { 
    if(n==='TODOS') filtrosEquipeAtivos=[]; 
        ...
        if(filtrosEquipeAtivos.includes(n)) filtrosEquipeAtivos=filtrosEquipeAtivos.filter(function(x){return x!==n;}); 
        else filtrosEquipeAtivos.push(n); 
    } 
    renderFiltrosEquipe(); renderQuadroEquipe(); 
}

function toggleColunaKanban(c) { 
    colsMinimizadas[c] = !colsMinimizadas[c]; 
    renderQuadroEquipe(); 
}

function renderQuadroEquipe() { 
    ['TODO','DOING','DONE'].forEach(function(c) { 
        const ct=getEl('col-'+c.toLowerCase()+'-container');
        const el=getEl('col-'+c.toLowerCase());
        const bt=getEl('btn-toggle-'+c.toLowerCase()); 
        
        if(ct&&el&&bt){ 
            if(colsMinimizadas[c]){ 
                ct.classList.remove('min-h-[500px]'); ct.classList.add('h-fit','pb-0'); el.classList.add('hidden'); bt.innerText='➕'; 
            } else { 
                ct.classList.add('min-h-[500px]'); ct.classList.remove('h-fit','pb-0'); el.classList.remove('hidden'); bt.innerText='➖'; 
            } 
        } 
    }); 
    ...
    let ord = [...tarefasEquipe].sort(function(a,b) { 
        let pA=pP[a.prazo||""]||6, pB=pP[b.prazo||""]||6; 
        return pA!==pB ? pA-pB : b.uid-a.uid; 
    }); 
    ...
    ord.forEach(function(t) { 
        ...
        if(mP[t.prazo]) { 
            let pts = mP[t.prazo].split(' '); 
            bdg = `<span class="${pts[0]} ${pts[1]} px-2 py-0.5 rounded text-[9px] font-black uppercase shadow-sm border">${pts[2]} ${pts[3]||''} ${pts[4]||''}</span>`; 
        }
    ...
    tarefasEquipe.forEach(function(t) { 
        const cx=getEl('chat-'+t.uid); if(cx) cx.scrollTop=cx.scrollHeight; 
    }); 
}
...
function adicionarTarefaEquipe() { 
    ...
    tarefasEquipe.push(nT); salvarColecao('tarefasEquipe', tarefasEquipe); 
    registrarAcao('📌', 'CRIOU TAREFA', `PARA: ${r}`); 
    
    getEl('eq_desc').value=""; 
    renderQuadroEquipe(); 
    setTimeout(function(){notificarNoGrupoComPrint(nT,'NOVA');}, 300); 
}

function moverTarefaEquipe(uid, nC) { 
    const t=tarefasEquipe.find(function(x){return x.uid==uid;}); 
    if(t && t.coluna!==nC) { 
        t.coluna=nC; salvarColecao('tarefasEquipe', tarefasEquipe); 
        renderQuadroEquipe(); 
...
function acionarUploadImagem(uid) { 
    if(!usuarioAtual) return alert("Identifique-se primeiro!"); 
    uidUploadPendente = uid; 
    getEl('file-upload-global').click(); 
}
...
function adicionarComentarioInline(uid, i) { 
    if(!usuarioAtual) return alert("Identifique-se!"); 
    const x = i.value.trim().toUpperCase(); if(!x) return; 
...
function cadastrarAssistencia(){ 
    ...
    if(c&&p){ 
        ...
        getEl('as_cliente').value=""; getEl('as_produto').value=""; 
    } 
...
function cycleAssisStatus(u){ 
    ...
    x.status = s[(s.indexOf(x.status||"Aguardando")+1)%s.length]; 
    salvarColecao('assistencias', assistencias); renderAssistencias(); 
}
