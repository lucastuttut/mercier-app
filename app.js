...
let tarefasEquipe=[], entregas=[], lembretes=[], historicoAtividades=[];
...
// ==========================================
// 4. CONEXÃO FIREBASE E DIÁRIO DE BORDO
// ==========================================
...
db.ref('dados').on('value', function(s) { 
    try { 
        const d = s.val() || {}; 
        pedidos=safeArray(d.pedidos); fornecedores=safeArray(d.fornecedores); estoque=safeArray(d.estoque); 
        catalogo=safeArray(d.catalogo); tarefas=safeArray(d.tarefas); assistencias=safeArray(d.assistencias); 
        tarefasEquipe=safeArray(d.tarefasEquipe); historicoAtividades=safeArray(d.historico); 
        entregas=safeArray(d.entregas); lembretes=safeArray(d.lembretes); 
...
function validarCPF(c){ 
    ...
    let a=0; for(let i=0;i<9;i++) a+=parseInt(c.charAt(i))*(10-i); let r=11-(a%11); if(r===10||r===11) r=0; if(r!==parseInt(c.charAt(9))) return false; 
...
function arquivarPedido(u) { 
    ...
    if(x){ x.finalizado = !x.finalizado; x.status = x.finalizado ? "Entregue/Finalizado" : "Pedido na loja"; salvarColecao('pedidos', pedidos); registrarAcao(x.finalizado ? '📦':'🔙', x.finalizado ? 'FINALIZOU':'RESTAUROU', `PEDIDO: ${x.cliente}`); renderPedidos(); } 
}

function excluirPedido(uid) { 
    const p = pedidos.find(function(x) { return x.uid == uid; }); if(!p) return; 
    if(p.finalizado) { if(confirm("RETORNAR pedido para ativos?")) { p.finalizado=false; p.status="Pedido na loja"; salvarColecao('pedidos', pedidos); renderPedidos(); } } 
    else { if(confirm("EXCLUIR PERMANENTEMENTE?")) { pedidos = pedidos.filter(function(x){ return x.uid != uid; }); salvarColecao('pedidos', pedidos); renderPedidos(); } } 
}

function cycleStatus(u){ 
    const x = pedidos.find(function(y){ return y.uid == u; }); if(!x || x.finalizado || x.status==="Entregue/Finalizado") return; 
    const s = ["Não enviado","Pedido enviado","Aguardando fábrica","Pedido na loja"]; x.status = s[(s.indexOf(x.status)+1)%s.length]; salvarColecao('pedidos', pedidos); 
}
...
function renderPedidos() { 
    ...
    lst.forEach(function(x) { 
        ...
        if(!x.finalizado && x.status !== "Entregue/Finalizado") { 
            btn += `<button onclick="arquivarPedido(${x.uid})" title="Finalizar">📦</button><button onclick="dupPed(${x.uid})" title="Duplicar">➕</button><button onclick="gerarAssistenciaRapida(${x.uid})" title="Assistência">🛠️</button><button onclick="excluirPedido(${x.uid})" class="text-red-500 font-black">✕</button>`; 
        } else { btn += `<button onclick="excluirPedido(${x.uid})" class="text-slate-400 font-black text-lg" title="Retornar">🔙</button>`; } 
...
function gerarEmailLote() {
    ...
    const selecionados = pedidos.filter(function(p) { return uids.includes(String(p.uid)); });
    ...
    selecionados.forEach(function(p) {
        ...
        texto += `REF: ID# ${p.idDoc.replace('ID#','').trim()}\n\n`;
    });
    
    texto += `Forma de pagamento: 30/60/90.\n\n`;
    texto += `IDs para controle interno, favor desconsiderar.\n\n`;
    texto += `Favor confirmar o recebimento e nos enviar o documento de confirmação dos itens acima para conferência.\n\n`;
    ...
    navigator.clipboard.writeText(texto).then(function() {
        if (confirm("✅ Texto gerado com as exigências!\nDeseja alterar o status dos pedidos para 'Pedido enviado'?")) {
            selecionados.forEach(function(p) { if(p.status === "Não enviado") p.status = "Pedido enviado"; });
...
// =========================================================
// 7. ABA: LOGÍSTICA & AGENDA UNIFICADA
// =========================================================
...
function renderCalendario() {
    ...
    let html = "";
    for (let i=0; i<pD; i++) html += `<div class="border-b border-r border-slate-200 bg-slate-50 opacity-40 min-h-[80px]"></div>`;
    ...
    const grid = getEl('grid-dias'); if(grid) grid.innerHTML = html;
}
    ...
    evs.forEach(function(ev) {
        if(ev.tipo === 'ENTREGA') {
            ...
            <div class="bg-white border-l-4 border-emerald-500 rounded-xl p-4 shadow-sm border border-slate-200 flex flex-col ${e.status==='Entregue'?'opacity-60':''}">
                ...
                <div class="flex justify-between items-center border-t border-slate-100 pt-3">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button onclick="editarEntrega(${e.uid})" class="bg-amber-50 text-amber-500 p-1.5 rounded-lg hover:bg-amber-100">✏️</button>
            ...
            <div class="bg-blue-50 border-l-4 border-blue-500 rounded-xl p-3 shadow-sm border border-blue-100">
                ...
                <p class="text-[9px] font-bold text-slate-500 uppercase mt-1">${p.qtd}x ${esc(p.produto)}</p>
            </div>`;
        }
    });
    pn.innerHTML = html;
...
function salvarEntrega() { 
    ...
    if(uid) {
        const it = entregas.find(function(x){ return x.uid == uid; });
        if(it) { it.dataOrdem = d; it.hora = h; it.cliente = c; it.endereco = e; it.produtos = p; it.contato = cont; }
    } else {
...
function excluirEntrega(uid) { if(confirm("Apagar entrega?")) { entregas = entregas.filter(function(x){ return x.uid != uid; }); salvarColecao('entregas', entregas); processarEventosCalendario(); } }
...
function mudarVisaoEstoque(v) { 
    visaoEstoque = v; 
    const bD = getEl('btn-estoque-disponivel'), bS = getEl('btn-estoque-saidas'); 
    if(v==='ESTOQUE') { 
...
function moverTarefaEquipe(uid, nC) { 
    ...
    if(t && t.coluna!==nC) { 
        t.coluna=nC; salvarColecao('tarefasEquipe', tarefasEquipe); renderQuadroEquipe(); 
        if(nC==='DONE') setTimeout(function(){notificarNoGrupoComPrint(t,'CONCLUIDA');},300); 
    } 
}
...
// =========================================================
// 10. TAREFAS / VENDAS E ASSISTÊNCIAS
// =========================================================
function processarFichaWhatsApp(tx) {
    ...
    if(mN) getEl('t_nome').value = mN[1].trim().toUpperCase(); 
    if(mE) getEl('t_end').value = mE[1].trim().toUpperCase(); 
    if(mC1) getEl('t_contato').value = mC1[1].trim().toUpperCase(); 
    if(mC2) getEl('t_contato2').value = mC2[1].trim().toUpperCase(); 
    if(mNu) getEl('t_num').value = mNu[1].trim().toUpperCase(); 
    if(mO) getEl('t_obs').value = mO[1].trim().toUpperCase();
    
    if(mCp) { getEl('t_cpf').value = mCp[1].trim(); maskCPF(getEl('t_cpf')); } 
    if(mCe) { getEl('t_cep').value = mCe[1].trim(); buscarCEP(getEl('t_cep')); } 
    if(mP) { let ps = mP[1].split('/'); if(ps.length===3) getEl('t_previsao').value = `${ps[2]}-${ps[1]}-${ps[0]}`; }
}
...
function addProdutoLinha(){ 
    const d = getEl('lista-produtos-tarefa'); 
    const r = document.createElement('div'); r.className = "flex flex-col md:flex-row gap-2 mb-2 items-start md:items-center row-prod bg-slate-50 p-3 rounded-lg border w-full"; 
...
function addPagamentoLinha(){ 
    const d = getEl('lista-pagamentos-tarefa'); let t=0, p=0; 
    document.querySelectorAll('.t-v-desc').forEach(function(i){ t+=parseMoney(i.value); }); 
    document.querySelectorAll('.t-p-val').forEach(function(i){ p+=parseMoney(i.value); }); 
    let s=t-p; if(s<0) s=0; 
...
function setP(b,v){ 
    const p=b.parentElement; p.querySelectorAll('button').forEach(function(x){ x.classList.remove('active'); }); 
    b.classList.add('active'); p.querySelector('.t-p-tipo').value=v; 
...
function calcTotalTirarPedido(){ 
    let t=0; document.querySelectorAll('.t-v-desc').forEach(function(i){ t+=parseMoney(i.value); }); 
    getEl('total-pedido-tarefa').innerText = "Total: R$ "+t.toLocaleString('pt-BR',{minimumFractionDigits:2}); 
}
...
function cadastrarTarefa(){ 
    ...
    if(t==='TIRAR PEDIDO'){ 
        ...
        document.querySelectorAll('.row-prod').forEach(function(r){ 
            if(r.querySelector('.t-p-nome').value) obj.detalhes.produtos.push({n:r.querySelector('.t-p-nome').value.toUpperCase(), o:r.querySelector('.t-v-orig').value, d:r.querySelector('.t-v-desc').value}); 
        }); 
        document.querySelectorAll('.row-pag').forEach(function(r){ 
            const tp=r.querySelector('.t-p-tipo').value, pc=(tp==='CRÉDITO')?r.querySelector('.t-p-parc').value:""; 
            obj.detalhes.pagamentos.push({t:tp+(pc?" "+pc:""), v:r.querySelector('.t-p-val').value, o:r.querySelector('.t-p-obs').value.toUpperCase()}); 
        }); 
    } else { 
...
function cycleAssisStatus(u){ 
    ...
    x.status = s[(s.indexOf(x.status||"Aguardando")+1)%s.length]; salvarColecao('assistencias', assistencias); renderAssistencias(); 
}

function renderFornecedores() { 
    const tb=getEl('tabelaFornecedores'); if(!tb) return; 
    tb.innerHTML=fornecedores.map(function(f,i){ return `<tr><td class="font-bold uppercase">${esc(f.nome)}</td><td class="lowercase text-blue-600">${esc(f.email)}</td><td class="text-center"><button onclick="fornecedores.splice(${i},1); salvarColecao('fornecedores', fornecedores);" class="text-red-500 font-black">✕</button></td></tr>`; }).join(''); 
}
function cadastrarFornecedor(){ 
    const n=val('f_nome').toUpperCase().trim(), e=val('f_email').toLowerCase().trim(); 
    if(n&&e){ fornecedores.push({nome:n,email:e}); salvarColecao('fornecedores', fornecedores); getEl('f_nome').value=""; getEl('f_email').value=""; } 
}
function renderCatalogo() { 
    const tb=getEl('tabelaCatalogo'); if(!tb) return; 
    tb.innerHTML=catalogo.map(function(c,i){ return `<tr><td class="uppercase">${esc(c.nome)}</td><td class="text-center"><button onclick="catalogo.splice(${i},1); salvarColecao('catalogo', catalogo);">✕</button></td></tr>`; }).join(''); 
}
function cadastrarCatalogo(){ 
    const n=val('cat_nome').toUpperCase(); 
    if(n){ catalogo.push({nome:n}); salvarColecao('catalogo', catalogo); getEl('cat_nome').value=""; } 
}
...
function atualizarSelectsFornecedores(){ 
    ...
    if(getEl('m_fornecedor_select')) getEl('m_fornecedor_select').innerHTML=h||"<option>...</option>"; 
    if(getEl('as_fabrica')) getEl('as_fabrica').innerHTML=h||"<option>...</option>"; 
    if(getEl('e_fabrica_select')) getEl('e_fabrica_select').innerHTML=h||"<option>...</option>"; 
    if(getEl('estoque-filtro-fabrica')) getEl('estoque-filtro-fabrica').innerHTML='<option value="TODAS">TODAS AS FÁBRICAS</option>'+h; 
}
function atualizarSugestoes(){ 
    const n = [...new Set(pedidos.map(function(p){ return p.cliente; }))].sort(); 
    if(getEl('listaSugestaoClientes')) getEl('listaSugestaoClientes').innerHTML=n.map(function(x){ return `<option value="${esc(x)}">`; }).join(''); 
}
function togglePainelSugestoes(){ 
    const p=getEl('painel-sugestoes'); p.style.display=(p.style.display==='flex')?'none':'flex'; 
}
function autoSalvarNotas(){ 
    notasMelhoria=val('texto-melhorias'); db.ref('dados/notasMelhoria').set(notasMelhoria); 
}
